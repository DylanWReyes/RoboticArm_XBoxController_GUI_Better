...
namespace RoboticArm_XBoxController_GUI
{
    ...
    public partial class Form1 : Form
    {
      ...
      void GimbalTracking(int xError, int yError)
      {
         ...
         if ((gimbalY < 315 || gimbalY > 325))
         {
             //changing gimbalY
             BottleY = 320 - BottleY;									// calculate y discrepancy
             if (BottleY > 0)
                 gimbalY = gimbalY + (Math.Abs(BottleY) / 320);			// increase gimbalY if y discrepancy < 0
             else
                 gimbalY = gimbalY - (Math.Abs(BottleY) / 320);			// decrease gimbalY if y discrepancy > 0
         }
